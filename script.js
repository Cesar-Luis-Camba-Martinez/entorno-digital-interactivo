...
/* =====================================================================
   RESOLUCIÓN PASO A PASO DIDÁCTICA Y PASO A PASO PARA PRINCIPIANTES
   ===================================================================== */
...
function resolverLineal() {
  ...
  if (a === 0) {
    res.innerHTML = '<span style="color:#ef4444; font-weight:bold;">Error matemático: Si $a = 0$, la variable $x$ desaparece y la expresión deja de ser una ecuación de primer grado.</span>';
    renderizarMatematicasGlobal();
    return;
  }
  ...
  let x = -b / a;
  res.innerHTML = `
    <div><strong>🎯 Objetivo:</strong> Dejar la variable $x$ completamente sola a un lado del signo igual ($=$).</div>
    <div><strong>Paso 1 (Planteamiento inicial):</strong> Escribimos la ecuación original con sus coeficientes:</div>
    <div style="margin-left: 1.5rem;">$${a}x + (${b}) = 0$</div>
    <div><strong>Paso 2 (Transposición de términos):</strong> Pasamos el término independiente $(${b})$ al lado derecho cambiando su signo (si está sumando pasa a restar, y viceversa):</div>
    <div style="margin-left: 1.5rem;">$${a}x = -(${b}) \\implies ${a}x = ${-b}$</div>
    <div><strong>Paso 3 (Despeje de la variable):</strong> El coeficiente $a = ${a}$ está multiplicando a $x$. Para despejar $x$, pasa al lado derecho dividiendo a todo el miembro:</div>
    <div style="margin-left: 1.5rem;">$x = \\frac{${-b}}{${a}}$</div>
    <div><strong>Paso 4 (Cálculo numérico final):</strong> Realizamos la división entre el numerador y el denominador:</div>
...
function resolverFraccionaria() {
  ...
  let restriccion = -b;
  let pasos = `<div><strong>🎯 Objetivo:</strong> Despejar la incógnita $x$ del denominador asegurando que la expresión sea válida en los números reales.</div>`;
  pasos += `<div><strong>Paso 1 (Análisis de Dominio y Restricción):</strong> En matemáticas no existe la división para cero. El denominador $x + (${b})$ no puede ser cero:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$x + (${b}) \\neq 0 \\implies x \\neq ${restriccion}$</div>`;

  if (c === 0) {
    pasos += `<div><strong>Paso 2 (Análisis del caso $c = 0$):</strong> Si $c = 0$, nos queda $\\frac{${a}}{x + (${b})} = 0$. Una fracción solo es cero si su numerador es cero, pero $${a} \\neq 0$.</div>`;
    pasos += `<div class="resultado-final" style="background-color:#fef2f2; border-color:#fecaca; color:#991b1b;">Conjunto Solución Vacío: $\\mathcal{S} = \\emptyset$</div>`;
    res.innerHTML = pasos;
    renderizarMatematicasGlobal();
    return;
  }
  ...
  pasos += `<div><strong>Paso 2 (Mover el término independiente):</strong> Transponemos el término libre $+(${c})$ al lado derecho restando:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$\\frac{${a}}{x + (${b})} = -(${c}) = ${-c}$</div>`;
  pasos += `<div><strong>Paso 3 (Despejar el denominador):</strong> Multiplicamos ambos lados por $(x + ${b})$ para quitarlo del denominador y pasamos $${-c}$ dividiendo:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$x + (${b}) = \\frac{${a}}{${-c}} \\implies x + (${b}) = ${numPaso2.toFixed(4)}$</div>`;
  pasos += `<div><strong>Paso 4 (Aislar $x$):</strong> Transponemos $+(${b})$ restando al lado derecho para obtener el valor final:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$x = ${numPaso2.toFixed(4)} - (${b}) \\implies x = ${x.toFixed(4)}$</div>`;

  if (Math.abs(x - restriccion) < 0.0001) {
    pasos += `<div class="resultado-final" style="background-color:#fef2f2; border-color:#fecaca; color:#991b1b;">La solución calculada ($x = ${x.toFixed(4)}$) coincide con la restricción de dominio ($x \\neq ${restriccion}$). Por lo tanto, no existe solución real.</div>`;
  } else {
    pasos += `<div class="resultado-final">Solución válida: $x = ${x.toFixed(4)}$ (Satisface la condición $x \\neq ${restriccion}$)</div>`;
  }
...
function calcularEcuacion() {
  ...
  let pasos = `<div><strong>🎯 Objetivo:</strong> Resolver $${a}x^2 + (${b})x + (${c}) = 0$ sustituyendo en la Fórmula General: $x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}$.</div>`;
  pasos += `<div><strong>Paso 1 (Identificación de coeficientes):</strong></div>`;
  pasos += `<div style="margin-left: 1.5rem;">• Coeficiente cuadrático ($a$): $a = ${a}$<br>• Coeficiente lineal ($b$): $b = ${b}$<br>• Término independiente ($c$): $c = ${c}$</div>`;
  
  pasos += `<div><strong>Paso 2 (Cálculo del Discriminante $\\Delta$):</strong> Evaluamos $\\Delta = b^2 - 4ac$ para saber el número de soluciones:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$\\Delta = (${b})^2 - 4(${a})(${c}) = ${bCuadrado} - (${cuatroAC}) = ${discriminante.toFixed(2)}$</div>`;
  ...
  if (discriminante > 0) {
    ...
    pasos += `<div><strong>Paso 3 (Sustitución en la Fórmula General):</strong> Como $\\Delta > 0$, existen dos soluciones reales distintas:</div>`;
    pasos += `<div style="margin-left: 1.5rem;">$x = \\frac{-(${b}) \\pm \\sqrt{${discriminante.toFixed(2)}}}{2(${a})} = \\frac{${menosB.toFixed(2)} \\pm ${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}}$</div>`;
    pasos += `<div><strong>Paso 4 (Separación de los signos $+$ y $-$):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">• Primer valor ($+$): $x_1 = \\frac{${menosB.toFixed(2)} + ${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}} = \\frac{${(menosB + raizDisc).toFixed(4)}}{${dosA.toFixed(2)}} = ${x1.toFixed(4)}$</div>`;
    pasos += `<div style="margin-left: 1.5rem;">• Segundo valor ($-$): $x_2 = \\frac{${menosB.toFixed(2)} - ${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}} = \\frac{${(menosB - raizDisc).toFixed(4)}}{${dosA.toFixed(2)}} = ${x2.toFixed(4)}$</div>`;
    pasos += `<div class="resultado-final">Dos soluciones reales distintas: $x_1 = ${x1.toFixed(4)}, \\quad x_2 = ${x2.toFixed(4)}$</div>`;

  } else if (discriminante === 0) {
    let x = menosB / dosA;
    pasos += `<div><strong>Paso 3 (Sustitución en la Fórmula General):</strong> Como $\\Delta = 0$, la raíz cuadrada es cero y se obtiene una sola solución real:</div>`;
    pasos += `<div style="margin-left: 1.5rem;">$x = \\frac{-(${b}) \\pm 0}{2(${a})} = \\frac{${menosB.toFixed(2)}}{${dosA.toFixed(2)}} = ${x.toFixed(4)}$</div>`;
    ...
    pasos += `<div><strong>Paso 3 (Análisis para $\\Delta < 0$):</strong> Como el discriminante es negativo, se utiliza la unidad imaginaria $i = \\sqrt{-1}$:</div>`;
    pasos += `<div style="margin-left: 1.5rem;">$\\sqrt{${discriminante.toFixed(2)}} = \\sqrt{${absDisc.toFixed(2)}} \\cdot i = ${raizDisc.toFixed(4)}i$</div>`;
    pasos += `<div><strong>Paso 4 (Expresión de la solución compleja $a \\pm bi$):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">$x = \\frac{${menosB.toFixed(2)} \\pm ${raizDisc.toFixed(4)}i}{${dosA.toFixed(2)}} = ${parteReal.toFixed(4)} \\pm ${parteImaginaria.toFixed(4)}i$</div>`;
  ...
  pasos += `<div style="margin-left: 1.5rem;">• <strong>Vértice $V(h, k)$:</strong> $h = \\frac{-b}{2a} = ${h.toFixed(4)}, \\quad k = f(h) = ${k.toFixed(4)} \\implies V(${h.toFixed(4)}, ${k.toFixed(4)})$</div>`;
  pasos += `<div style="margin-left: 1.5rem;">• <strong>Eje de simetría:</strong> $x = ${h.toFixed(4)}$</div>`;
  pasos += `<div style="margin-left: 1.5rem;">• <strong>Concavidad:</strong> ${concavidad} (${tipoExtremo})</div>`;
  pasos += `<div style="margin-left: 1.5rem;">• <strong>Corte en eje Y:</strong> $(0, ${c})$</div>`;
  pasos += `<div style="margin-left: 1.5rem;">• <strong>Rango de la función:</strong> $\\text{Rec}(f) = ${rangoStr}$</div>`;
...
function resolverSistema() {
  ...
  let pasos = `<div><strong>🎯 Objetivo:</strong> Encontrar las coordenadas del punto de corte $(x, y)$ entre dos rectas usando la Regla de Cramer.</div>`;
  pasos += `<div><strong>Recordatorio de determinantes $2 \\times 2$:</strong> Se multiplican los datos en cruz y se restan: $\\begin{vmatrix} p & q \\\\ r & s \\end{vmatrix} = (p \\cdot s) - (q \\cdot r)$.</div>`;

  pasos += `<div><strong>Paso 1 (Determinante Principal $D$):</strong> Se organiza con los coeficientes de $x$ e $y$:</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$D = \\begin{vmatrix} ${a1} & ${b1} \\\\ ${a2} & ${b2} \\end{vmatrix} = (${a1})(${b2}) - (${b1})(${a2}) = ${a1 * b2} - (${b1 * a2}) = ${D.toFixed(4)}$</div>`;

  pasos += `<div><strong>Paso 2 (Determinante en $x$, $D_x$):</strong> Reemplazamos la columna de $x$ por los términos independientes ($c_1, c_2$):</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$D_x = \\begin{vmatrix} ${c1} & ${b1} \\\\ ${c2} & ${b2} \\end{vmatrix} = (${c1})(${b2}) - (${b1})(${c2}) = ${c1 * b2} - (${b1 * c2}) = ${Dx.toFixed(4)}$</div>`;

  pasos += `<div><strong>Paso 3 (Determinante en $y$, $D_y$):</strong> Reemplazamos la columna de $y$ por los términos independientes ($c_1, c_2$):</div>`;
  pasos += `<div style="margin-left: 1.5rem;">$D_y = \\begin{vmatrix} ${a1} & ${c1} \\\\ ${a2} & ${c2} \\end{vmatrix} = (${a1})(${c2}) - (${c1})(${a2}) = ${a1 * c2} - (${c1 * a2}) = ${Dy.toFixed(4)}$</div>`;

  if (Math.abs(D) > 1e-6) {
    let x = Dx / D;
    let y = Dy / D;
    pasos += `<div><strong>Paso 4 (Cálculo final de $x$ e $y$):</strong> Aplicamos las fórmulas $x = \\frac{D_x}{D}$ e $y = \\frac{D_y}{D}$:</div>`;
    ...
    pasos += `<div class="resultado-final">Sistema Compatible Determinado (Punto de corte único): $x = ${x.toFixed(4)}, \\quad y = ${y.toFixed(4)}$</div>`;
  } else {
    if (Math.abs(Dx) < 1e-6 && Math.abs(Dy) < 1e-6) {
      pasos += `<div><strong>Paso 4 (Interpretación):</strong> Al dar $D=0$, $D_x=0$ y $D_y=0$, ambas ecuaciones representan exactamente la misma recta.</div>`;
      ...
      pasos += `<div><strong>Paso 4 (Interpretación):</strong> Al dar $D=0$ pero $D_x \\neq 0$ o $D_y \\neq 0$, las dos rectas son paralelas y jamás se cruzan.</div>`;
...
function resolverAbsoluto() {
  ...
  let pasos = `<div><strong>🎯 Objetivo:</strong> Resolver $|${a}x + (${b})| = ${c}$.</div>`;
  pasos += `<div><strong>Concepto clave:</strong> El valor absoluto representa una distancia física en la recta real. Las distancias no pueden ser valores negativos.</div>`;

  if (c < 0) {
    pasos += `<div><strong>Paso 1 (Comprobación de viabilidad):</strong> Como el valor de la distancia $c = ${c}$ es menor a cero, la ecuación no tiene solución.</div>`;
    ...
    pasos += `<div><strong>Paso 1 (Descomposición en dos ecuaciones lineales):</strong> La expresión dentro de las barras puede valer $+${c}$ o $-${c}$:</div>`;
    pasos += `<div style="margin-left: 1.5rem;">• <strong>Caso A (Positivo):</strong> $${a}x + (${b}) = ${c}$</div>`;
    pasos += `<div style="margin-left: 1.5rem;">• <strong>Caso B (Negativo):</strong> $${a}x + (${b}) = -${c}$</div>`;

    pasos += `<div><strong>Paso 2 (Despejar el Caso A):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">$${a}x = ${c} - (${b}) \\implies ${a}x = ${c - b} \\implies x_1 = \\frac{${c - b}}{${a}} = ${x1.toFixed(4)}$</div>`;

    pasos += `<div><strong>Paso 3 (Despejar el Caso B):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">$${a}x = -${c} - (${b}) \\implies ${a}x = ${-c - b} \\implies x_2 = \\frac{${-c - b}}{${a}} = ${x2.toFixed(4)}$</div>`;
...
function resolverPolinomioGenerico(coefs, nombreGrado) {
  ...
  let pasos = `<div><strong>🎯 Objetivo:</strong> Determinar todas las raíces de la ecuación polinómica de ${nombreGrado}.</div>`;
  pasos += `<div><strong>Explicación de la Regla de Ruffini:</strong> Se utiliza para reducir progresivamente el grado del polinomio dividiendo para una raíz $x = r$. En la tabla: (1) Se baja el primer número, (2) Se multiplica por $r$, (3) Se suma con el número de la siguiente columna hasta lograr residuo $0$.</div>`;
  ...
  if (coefsActuales.length === 3) {
    ...
    let dosA = 2 * A;
    let menosB = -B;
    let idx1 = raices.length + 1;
    let idx2 = raices.length + 2;

    pasos += `<div><strong>Paso ${pasoNum} (Despeje mediante Fórmula Cuadrática del cociente reducido $(${A})x^2 + (${B})x + (${C}) = 0$):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">• <strong>Identificación de coeficientes:</strong> $a = ${A}, \\quad b = ${B}, \\quad c = ${C}$</div>`;
    pasos += `<div style="margin-left: 1.5rem;">• <strong>Cálculo del Discriminante ($\\Delta$):</strong> $\\Delta = b^2 - 4ac = (${B})^2 - 4(${A})(${C}) = ${(B*B)} - (${4*A*C}) = ${disc.toFixed(4)}$</div>`;
    pasos += `<div style="margin-left: 1.5rem;">• <strong>Sustitución en la Fórmula General:</strong> $x = \\frac{-b \\pm \\sqrt{\\Delta}}{2a} = \\frac{-(${B}) \\pm \\sqrt{${disc.toFixed(4)}}}{2(${A})}$</div>`;
    ...
    if (disc > 0) {
      ...
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Despeje de la primera raíz ($+$):</strong> $x_{${idx1}} = \\frac{${menosB.toFixed(2)} + ${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}} = \\frac{${(menosB + raizDisc).toFixed(4)}}{${dosA.toFixed(2)}} = ${r1.toFixed(4)}$</div>`;
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Despeje de la segunda raíz ($-$):</strong> $x_{${idx2}} = \\frac{${menosB.toFixed(2)} - ${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}} = \\frac{${(menosB - raizDisc).toFixed(4)}}{${dosA.toFixed(2)}} = ${r2.toFixed(4)}$</div>`;
      raices.push(`x_{${idx1}} = ${r1.toFixed(4)}`);
      raices.push(`x_{${idx2}} = ${r2.toFixed(4)}`);
    } else if (Math.abs(disc) < 1e-6) {
      let r = menosB / dosA;
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Despeje de raíz única ($\\Delta = 0$):</strong> $x_{${idx1}} = x_{${idx2}} = \\frac{${menosB.toFixed(2)}}{${dosA.toFixed(2)}} = ${r.toFixed(4)}$</div>`;
      raices.push(`x_{${idx1}} = ${r.toFixed(4)}`);
    } else {
      let absDisc = -disc;
      let raizDisc = Math.sqrt(absDisc);
      let parteReal = menosB / dosA;
      ...
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Despeje en el campo complejo ($\\Delta < 0$):</strong> $\\sqrt{${disc.toFixed(4)}} = \\sqrt{${absDisc.toFixed(4)}}i = ${raizDisc.toFixed(4)}i$</div>`;
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Raíz compleja 1:</strong> $x_{${idx1}} = \\frac{${menosB.toFixed(2)}}{${dosA.toFixed(2)}} + \\frac{${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}}i = ${parteReal.toFixed(4)} + ${parteImaginaria.toFixed(4)}i$</div>`;
      pasos += `<div style="margin-left: 1.5rem;">• <strong>Raíz compleja 2:</strong> $x_{${idx2}} = \\frac{${menosB.toFixed(2)}}{${dosA.toFixed(2)}} - \\frac{${raizDisc.toFixed(4)}}{${dosA.toFixed(2)}}i = ${parteReal.toFixed(4)} - ${parteImaginaria.toFixed(4)}i$</div>`;
      raices.push(`x_{${idx1}} = ${parteReal.toFixed(4)} + ${parteImaginaria.toFixed(4)}i`);
      raices.push(`x_{${idx2}} = ${parteReal.toFixed(4)} - ${parteImaginaria.toFixed(4)}i`);
    }
  } else if (coefsActuales.length > 3) {
    pasos += `<div><strong>Paso ${pasoNum} (Polinomio no reducible por raíces racionales):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem; color: #b91c1c;">El polinomio cociente de grado ${coefsActuales.length - 1} carece de raíces racionales enteras o fraccionarias exactas mediante la Regla de Ruffini.</div>`;
  }
