...
function dibujarPlano() {
  ...
  // 4. Marcas y graduación numérica
...
function extraerCantidadDeclaradaPorEstudiante(texto) {
  ...
  return [...new Set(numeros.map(n => Math.round(n * 1000) / 1000))].length;
}
...
function verificarSustitucionSistema(intento, coef) {
  const cX = intento.match(/\bx\s*=\s*(-?\d+(?:[.,]\d+)?)/i);
  const cY = intento.match(/\by\s*=\s*(-?\d+(?:[.,]\d+)?)/i);
  ...
  const x = parseFloat(cX[1].replace(',', '.'));
  const y = parseFloat(cY[1].replace(',', '.'));
...
function resolverLineal() {
  ...
  res.innerHTML = `<div><strong>Explicación:</strong> Para resolver la ecuación lineal $${a}x + (${b}) = 0$, se aísla la variable $x$.</div>
                   <div><strong>Paso 1: Planteamiento inicial:</strong> $${a}x + (${b}) = 0$</div>
                   <div><strong>Paso 2: Transposición del término independiente:</strong> $${a}x = ${-b}$</div>
                   <div><strong>Paso 3: Despeje formal:</strong> $x = \\frac{${-b}}{${a}}$</div>
...
/* =====================================================================
   HELPER DE RUFFINI Y RESOLUCIÓN DE POLINOMIOS (CÚBICA, CUÁRTICA, ETC)
   ===================================================================== */
