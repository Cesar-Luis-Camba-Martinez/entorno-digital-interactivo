...
/* =====================================================================
   PROCESO PREVIO Y VERIFICACIÓN (VERSIÓN CORREGIDA)
   ===================================================================== */
...
function verificarIntentoEstudiante(intento, contenedorResultado, tipoEcuacion) {
  ...
  // Verificación inteligente de cantidad para evitar falsos negativos
  let cajaCantidad = '';
...
function resolverLineal() {
  ...
  if (a === 0) {
    res.innerHTML = '<span style="color:#ef4444; font-weight:bold;">Error matemático: Si $a = 0$, la variable $x$ desaparece.</span>';
    renderizarMatematicasGlobal();
    return;
  }
  let x = -b / a;
  res.innerHTML = `
    <div><strong>🎯 Objetivo:</strong> Dejar la variable $x$ completamente sola.</div>
    <div><strong>Paso 1:</strong> Escribimos la ecuación original: $${a}x + (${b}) = 0$</div>
    <div><strong>Paso 2:</strong> Transponemos $(${b})$ al lado derecho: $${a}x = ${-b}$</div>
    <div><strong>Paso 3:</strong> Despejamos $x$: $x = \\frac{${-b}}{${a}}$</div>
...
function resolverFraccionaria() {
  ...
  let restriccion = -b;
  let pasos = `<div><strong>🎯 Objetivo:</strong> Despejar la incógnita asegurando que el denominador no sea cero ($x \\neq ${restriccion}$).</div>`;
  if (c === 0) {
  ...
  let numPaso2 = -a / c;
  let x = numPaso2 - b;
  pasos += `<div><strong>Paso 1:</strong> Restricción: $x \\neq ${restriccion}$</div>`;
  pasos += `<div><strong>Paso 2:</strong> Despejando: $x = ${x.toFixed(4)}$</div>`;
  pasos += `<div class="resultado-final">Solución válida: $x = ${x.toFixed(4)}$</div>`;
  res.innerHTML = pasos;
  renderizarMatematicasGlobal();
}
...
function calcularEcuacion() {
  ...
  let pasos = `<div><strong>🎯 Objetivo:</strong> Resolver $${a}x^2 + (${b})x + (${c}) = 0$ mediante la Fórmula General.</div>`;
  pasos += `<div><strong>Discriminante:</strong> $\\Delta = ${discriminante.toFixed(2)}$</div>`;
...
function resolverSistema() {
  ...
  let Dx = c1 * b2 - c2 * b1;
  let Dy = a1 * c2 - a2 * c1;
  let pasos = `<div><strong>Regla de Cramer:</strong> $D = ${D}$, $D_x = ${Dx}$, $D_y = ${Dy}$</div>`;

  if (Math.abs(D) > 1e-6) {
    let x = Dx / D, y = Dy / D;
    pasos += `<div class="resultado-final">Solución única: $x = ${x.toFixed(4)}, \\quad y = ${y.toFixed(4)}$</div>`;
  } else {
    pasos += `<div class="resultado-final" style="background-color:#fffbeb; color:#92400e;">Sistema sin solución única (Paralelas o coincidentes)</div>`;
  }
  res.innerHTML = pasos;
  renderizarMatematicasGlobal();
}
...
function resolverAbsoluto() {
  ...
  if (c < 0) {
    res.innerHTML = `<div class="resultado-final" style="background-color:#fef2f2; color:#991b1b;">Sin solución real: $\\mathcal{S} = \\emptyset$</div>`;
    renderizarMatematicasGlobal();
    return;
  }
  let x1 = (c - b) / a;
  let x2 = (-c - b) / a;
  res.innerHTML = `<div class="resultado-final">Dos soluciones válidas: $x_1 = ${x1.toFixed(4)}, \\quad x_2 = ${x2.toFixed(4)}$</div>`;
  renderizarMatematicasGlobal();
}
...
function resolverPolinomioGenerico(coefs, nombreGrado) {
  ...
  if (coefsActuales.length === 3) {
    ...
    if (disc >= 0) {
      ...
      raices.push(`x = ${r1.toFixed(4)}`);
      raices.push(`x = ${r2.toFixed(4)}`);
    } else {
      raices.push(`raíces complejas`);
    }
  }
