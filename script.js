...
function calcularEcuacion() {
  ...
  if (discriminante > 0) {
  ...
  } else if (Math.abs(discriminante) < 0.000001) {
    let x = -b / (2 * a);
    ...
    let parteReal = -b / (2 * a);
    let parteImaginaria = Math.sqrt(-discriminante) / Math.abs(2 * a);
...
function resolverSistema() {
  ...
  pasos += `<div><strong>Paso 1: Determinante del Sistema:</strong></div>
               <div>$$\\text{Det}(S) = \\begin{vmatrix} ${a1} & ${b1} \\\\ ${a2} & ${b2} \\end{vmatrix} = (${a1} \\cdot ${b2}) - (${a2} \\cdot ${b1}) = ${detS.toFixed(4)}$$</div>`;

  if (Math.abs(detS) < 1e-9) {
    let detX = (c1 * b2) - (c2 * b1);
    let detY = (a1 * c2) - (a2 * c1);
    if (Math.abs(detX) < 1e-9 && Math.abs(detY) < 1e-9) {
      pasos += `<div style="color:#2563eb; font-weight:bold; margin-top:0.5rem;">Sistema Compatible Indeterminado: El determinante es cero y las rectas son coincidentes (Infinitas soluciones).</div>`;
    } else {
      pasos += `<div style="color:#ef4444; font-weight:bold; margin-top:0.5rem;">Sistema Incompatible: El determinante es cero y las rectas son paralelas distintas (Sin solución en $$\\mathbb{R}$$).</div>`;
    }
  } else {
    let detX = (c1 * b2) - (c2 * b1);
    let detY = (a1 * c2) - (a2 * c1);
    ...
    pasos += `<div><strong>Paso 2: Determinantes de las Incógnitas:</strong></div>
              <div>$$\\text{Det}(X) = \\begin{vmatrix} ${c1} & ${b1} \\\\ ${c2} & ${b2} \\end{vmatrix} = ${detX.toFixed(4)} \\quad | \\quad \\text{Det}(Y) = \\begin{vmatrix} ${a1} & ${c1} \\\\ ${a2} & ${c2} \\end{vmatrix} = ${detY.toFixed(4)}$$</div>
...
function obtenerDivisores(num) {
  let divisores = [];
  let absN = Math.round(Math.abs(num));
  if (absN === 0) return [0];
...
/* =====================================================================
   FUNCIONES AUXILIARES PARA REDUCCIÓN SINTÉTICA
   ===================================================================== */
function resolverCuarticaAuxiliar(a, b, c, d, e, indiceInicio) {
  ...
  if (e === 0) {
    html += `<div>Término independiente nulo ($$e=0$$). Se factoriza $$x$$:</div>`;
    html += `<div class="resultado-final">$$x_${indiceInicio} = 0$$</div>`;
    return html + resolverCubicaAuxiliar(a, b, c, d, indiceInicio + 1);
  }
  ...
  return html + resolverCubicaAuxiliar(a, c2, c3, c4, indiceInicio + 1);
}

function resolverCubicaAuxiliar(a, b, c, d, indiceInicio) {
  let html = `<div style="margin-top:1rem;"><strong>Resolución del Polinomio Cúbico Reducido:</strong></div>`;
  
  if (d === 0) {
    html += `<div>Término independiente nulo ($$d=0$$). Se factoriza $$x$$:</div>`;
    html += `<div class="resultado-final">$$x_${indiceInicio} = 0$$</div>`;
    return html + resolverCuadraticaResidual(a, b, c, indiceInicio + 1);
  }
  ...
  html += `<div>Evaluando $$x = ${r}$$: Raíz hallada: <span class="resultado-final">$$x_${indiceInicio} = ${r}$$</span></div>`;
  html += `<div class="tabla-ruffini-container">
    <table class="tabla-ruffini">
      <tr>
        <td class="col-raiz">x = ${r}</td>
        <td>${a}</td><td>${b}</td><td>${c}</td><td>${d}</td>
      </tr>
      <tr>
        <td class="col-raiz">↓</td>
  ...
  return html + resolverCuadraticaResidual(a, c2, c3, indiceInicio + 1);
}

function resolverCuadraticaResidual(a, b, c, indiceInicio) {
  let html = `<div style="margin-top:1rem;"><strong>Resolución de la Ecuación Cuadrática Residual:</strong> $$(${a})x^2 + (${b})x + (${c}) = 0$$</div>`;
  let disc = b * b - 4 * a * c;
  
  if (disc > 0) {
    let x1 = (-b + Math.sqrt(disc)) / (2 * a);
    let x2 = (-b - Math.sqrt(disc)) / (2 * a);
    html += `<div class="resultado-final">$$x_${indiceInicio} = ${x1.toFixed(4)}$$</div>`;
    html += `<div class="resultado-final">$$x_${indiceInicio + 1} = ${x2.toFixed(4)}$$</div>`;
  } else if (Math.abs(disc) < 0.000001) {
    let x = -b / (2 * a);
    html += `<div class="resultado-final">$$x_${indiceInicio} = ${x.toFixed(4)}$$ (Raíz doble)</div>`;
  } else {
    let real = -b / (2 * a);
    let imag = Math.sqrt(-disc) / Math.abs(2 * a);
    html += `<div class="resultado-final">$$x_${indiceInicio} = ${real.toFixed(2)} + ${imag.toFixed(2)}i$$</div>`;
    html += `<div class="resultado-final">$$x_${indiceInicio + 1} = ${real.toFixed(2)} - ${imag.toFixed(2)}i$$</div>`;
  }
  return html;
}
...
function resolverAbsoluto() {
  ...
  let pasos = `<div><strong>Explicación:</strong> Para resolver una ecuación con valor absoluto de la forma $$|ax + b| = c$$, se parte de la definición: $$|X| \\ge 0$$ para todo $$X \\in \\mathbb{R}$$, y $$|X| = c \\iff X = c \\;\\lor\\; X = -c$$ cuando $$c \\ge 0$$. Por ello, el procedimiento inicia analizando el signo del término externo $$c$$.</div>`;
  pasos += `<div><strong>Paso 1: Planteamiento de la ecuación original:</strong> $$|${a}x + (${b})| = ${c}$$</div>`;
  pasos += `<div><strong>Paso 2: Análisis del signo del término externo $$c = ${c}$$:</strong></div>`;

  if (c < 0) {
    pasos += `<div><strong>Paso 3: Justificación mediante la propiedad de no negatividad:</strong> Toda expresión de valor absoluto cumple $$|${a}x + (${b})| \\ge 0$$ para cualquier $$x \\in \\mathbb{R}$$. Como $$c = ${c} < 0$$, ninguna expresión de valor absoluto puede igualar un número negativo.</div>`;
    pasos += `<div class="resultado-final" style="background-color:#fef2f2; border-color:#fecaca; color:#991b1b;">La ecuación no posee solución real: $$\\mathcal{S} = \\emptyset$$</div>`;
  } else if (c === 0) {
    pasos += `<div><strong>Paso 3: Reducción a una única ecuación lineal equivalente:</strong> Como $$c = 0$$ y el único número real cuyo valor absoluto es cero es el propio cero, los dos casos de la propiedad fundamental coinciden en una sola igualdad:</div>`;
    pasos += `<div>$$${a}x + (${b}) = 0$$</div>`;
    let x = -b / a;
    pasos += `<div><strong>Paso 4: Transposición del término independiente:</strong> $$${a}x = ${-b}$$</div>`;
    pasos += `<div><strong>Paso 5: Despeje de la incógnita $$x$$ dividiendo para $$a$$ ($$a = ${a} \\neq 0$$):</strong> $$x = \\frac{${-b}}{${a}} = ${x.toFixed(4)}$$</div>`;
    let comp = a * x + b;
    pasos += `<div><strong>Paso 6: Verificación por sustitución en la ecuación original:</strong> $$|${a}(${x.toFixed(4)}) + (${b})| = |${comp.toFixed(4)}| = ${Math.abs(comp).toFixed(4)}$$ ✓</div>`;
    pasos += `<div class="resultado-final">$$x = ${x.toFixed(4)} \\qquad \\mathcal{S} = \\{${x.toFixed(4)}\\}$$</div>`;
  } else {
    pasos += `<div>Como $$c = ${c} > 0$$, se aplica la <strong>Propiedad Fundamental del Valor Absoluto</strong>: $$|X| = c \\iff X = c \\;\\lor\\; X = -c$$. Esta equivalencia genera dos ecuaciones lineales independientes que deben resolverse por separado:</div>`;
    pasos += `<div><strong>Paso 3: Planteamiento de las dos ecuaciones equivalentes:</strong></div>`;
    pasos += `<div>$$${a}x + (${b}) = ${c} \\;\\; \\text{(Caso 1)} \\qquad ${a}x + (${b}) = ${-c} \\;\\; \\text{(Caso 2)}$$</div>`;

    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 4: Transposición del término independiente (Caso 1, rama $$+c$$):</strong></div>`;
    let transp1 = c - b;
    pasos += `<div>$$${a}x = ${c} - (${b}) = ${transp1}$$</div>`;

    pasos += `<div><strong>Paso 5: Despeje de la incógnita $$x$$ dividiendo para $$a = ${a}$$ (Caso 1):</strong></div>`;
    let x1 = transp1 / a;
    pasos += `<div>$$x_1 = \\frac{${transp1}}{${a}} = ${x1.toFixed(4)}$$</div>`;

    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 6: Transposición del término independiente (Caso 2, rama $$-c$$):</strong></div>`;
    let transp2 = -c - b;
    pasos += `<div>$$${a}x = ${-c} - (${b}) = ${transp2}$$</div>`;

    pasos += `<div><strong>Paso 7: Despeje de la incógnita $$x$$ dividiendo para $$a = ${a}$$ (Caso 2):</strong></div>`;
    let x2 = transp2 / a;
    pasos += `<div>$$x_2 = \\frac{${transp2}}{${a}} = ${x2.toFixed(4)}$$</div>`;

    let comp1 = a * x1 + b;
    let comp2 = a * x2 + b;
    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 8: Verificación de ambas soluciones por sustitución en la ecuación original:</strong></div>`;
    pasos += `<div>Con $$x_1 = ${x1.toFixed(4)}$$: $$|${a}(${x1.toFixed(4)}) + (${b})| = |${comp1.toFixed(4)}| = ${Math.abs(comp1).toFixed(4)}$$ ✓</div>`;
    pasos += `<div>Con $$x_2 = ${x2.toFixed(4)}$$: $$|${a}(${x2.toFixed(4)}) + (${b})| = |${comp2.toFixed(4)}| = ${Math.abs(comp2).toFixed(4)}$$ ✓</div>`;

    pasos += `<div class="resultado-final">$$x_1 = ${x1.toFixed(4)}$$</div>`;
    pasos += `<div class="resultado-final">$$x_2 = ${x2.toFixed(4)}$$</div>`;
    pasos += `<div class="resultado-final">$$\\mathcal{S} = \\{${x1.toFixed(4)}, \\; ${x2.toFixed(4)}\\}$$</div>`;
  }
