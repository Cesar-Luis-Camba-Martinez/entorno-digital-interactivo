...
function resolverPolinomioGenerico(coefs, nombreGrado) {
  ...
  if (coefsActuales.length === 3) {
    ...
    let disc = B * B - 4 * A * C;
    pasos += `<div><strong>Paso ${pasoNum} (Resolución del polinomio reducido $(${A})x^2 + (${B})x + (${C}) = 0$):</strong></div>`;
    pasos += `<div style="margin-left: 1.5rem;">Como el polinomio ya quedó de grado 2, usamos directamente la fórmula cuadrática con $a=${A}, b=${B}, c=${C}$:</div>`;

    if (disc >= 0) {
      let r1 = (-B + Math.sqrt(disc)) / (2 * A);
      let r2 = (-B - Math.sqrt(disc)) / (2 * A);
      pasos += `<div style="margin-left: 1.5rem;">$\\Delta = (${B})^2 - 4(${A})(${C}) = ${disc.toFixed(4)}$</div>`;
      pasos += `<div style="margin-left: 1.5rem;">$x = \\frac{-(${B}) \\pm \\sqrt{${disc.toFixed(4)}}}{2(${A})} \\implies x_{${raices.length + 1}} = ${r1.toFixed(4)}, \\quad x_{${raices.length + 2}} = ${r2.toFixed(4)}$</div>`;
      ...
      pasos += `<div style="margin-left: 1.5rem;">$\\Delta = ${disc.toFixed(4)} < 0 \\implies \\text{Raíces complejas}$</div>`;
      pasos += `<div style="margin-left: 1.5rem;">$x = ${re} \\pm ${im}i$</div>`;
