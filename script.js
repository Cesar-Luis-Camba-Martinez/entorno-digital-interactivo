...
function resolverAbsoluto() {
  ...
  let pasos = `<div><strong>Explicación:</strong> Para resolver una ecuación con valor absoluto de la forma $$|ax + b| = c$$, primero se analiza el signo del término externo $$c$$, ya que toda expresión de valor absoluto es, por definición, no negativa.</div>`;
  ...
  if (c < 0) {
    pasos += `<div>Toda expresión de valor absoluto cumple $$|${a}x + (${b})| \\ge 0$$ para cualquier $$x \\in \\mathbb{R}$$. Como $$c = ${c} < 0$$, ninguna expresión de valor absoluto puede igualar un número negativo.</div>`;
    ...
    pasos += `<div>Como $$c = 0$$ y el único número real cuyo valor absoluto es cero es el propio cero, los dos casos de la propiedad fundamental coinciden y la ecuación se reduce a una única igualdad:</div>`;
    pasos += `<div>$$${a}x + (${b}) = 0$$</div>`;
    let x = -b / a;
    pasos += `<div><strong>Paso 3: Transposición del término independiente:</strong> $$${a}x = ${-b}$$</div>`;
    pasos += `<div><strong>Paso 4: Despeje formal dividiendo para $$a$$ ($$a = ${a} \\neq 0$$):</strong> $$x = \\frac{${-b}}{${a}} = ${x.toFixed(4)}$$</div>`;
    let comp = a * x + b;
    pasos += `<div><strong>Paso 5: Verificación por sustitución en la ecuación original:</strong> $$|${a}(${x.toFixed(4)}) + (${b})| = |${comp.toFixed(4)}| = ${Math.abs(comp).toFixed(4)}$$ ✓</div>`;
    pasos += `<div class="resultado-final">$$x = ${x.toFixed(4)} \\qquad \\mathcal{S} = \\{${x.toFixed(4)}\\}$$</div>`;
  } else {
    pasos += `<div>Como $$c = ${c} > 0$$, se aplica la <strong>Propiedad Fundamental del Valor Absoluto</strong>: $$|X| = c \\iff X = c \\;\\lor\\; X = -c$$. Esta equivalencia genera dos ecuaciones lineales independientes:</div>`;
    pasos += `<div>$$${a}x + (${b}) = ${c} \\qquad \\text{o bien} \\qquad ${a}x + (${b}) = ${-c}$$</div>`;

    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 3: Resolución del Caso 1 (rama positiva, $$+c$$):</strong></div>`;
    pasos += `<div>Planteamiento: $$${a}x + (${b}) = ${c}$$</div>`;
    let transp1 = c - b;
    pasos += `<div>Transposición del término independiente: $$${a}x = ${c} - (${b}) = ${transp1}$$</div>`;
    let x1 = transp1 / a;
    pasos += `<div>Despeje dividiendo para $$a = ${a}$$: $$x_1 = \\frac{${transp1}}{${a}} = ${x1.toFixed(4)}$$</div>`;

    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 4: Resolución del Caso 2 (rama negativa, $$-c$$):</strong></div>`;
    pasos += `<div>Planteamiento: $$${a}x + (${b}) = ${-c}$$</div>`;
    let transp2 = -c - b;
    pasos += `<div>Transposición del término independiente: $$${a}x = ${-c} - (${b}) = ${transp2}$$</div>`;
    let x2 = transp2 / a;
    pasos += `<div>Despeje dividiendo para $$a = ${a}$$: $$x_2 = \\frac{${transp2}}{${a}} = ${x2.toFixed(4)}$$</div>`;

    let comp1 = a * x1 + b;
    let comp2 = a * x2 + b;
    pasos += `<div style="margin-top:0.75rem;"><strong>Paso 5: Verificación de ambas soluciones por sustitución:</strong></div>`;
