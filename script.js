...
function resolverAbsoluto() {
  ...
  pasos += `<div>Por definición, si $|X| = c$ con $c > 0$, entonces $X = c$ o $X = -c$. Tomando $X = ${a}x + (${b})$, la ecuación se descompone en dos ecuaciones lineales:</div>`;
  pasos += `<div>• Caso 1 (la expresión dentro del valor absoluto es positiva): $${a}x + (${b}) = ${c}$</div>`;
  pasos += `<div>• Caso 2 (la expresión dentro del valor absoluto es negativa): $${a}x + (${b}) = ${-c}$</div>`;
