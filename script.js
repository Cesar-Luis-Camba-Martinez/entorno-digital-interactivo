...
function revelarSolucion() {
  ...
  if ((intento || procedimiento) && res) {
    ...
    // La validación automática SOLO usa la respuesta final, nunca el procedimiento libre,
    // para evitar que pasos intermedios (ej. "2x=4", "x=4/2") se confundan con soluciones adicionales.
    if (intento) {
...
function extraerNumerosDelIntento(texto) {
  ...
  // Elimina frases que declaran la CANTIDAD de soluciones (ej. "2 soluciones", "3 raíces").
  // Sin esto, el "2" de "tiene 2 soluciones" se confundía con un valor de x, generando
  // falsos "incorrecta" en ecuaciones con más de una solución (cuadrática, valor absoluto, etc.)
  // cuando el estudiante solo declaraba la cantidad correcta sin repetir los valores.
  const textoLimpio = texto.replace(/\d+\s*(soluci[oó]n(?:es)?|ra[ií]z|ra[ií]ces)/gi, ' ');
  ...
  while ((coincidencia = regexAsignacion.exec(textoLimpio)) !== null) {
  ...
  if (explicitos.length > 0) {
    ...
    const coincidencias = textoLimpio.match(regexGeneral) || [];
