...
function resolverPolinomica() {
  ...
  pasos += `<div>• Posibles raíces ($$p/q$$): {${candidatos.map(v => Number(v.toFixed(2))).join(', ')}}</div>`;
...
function resolverCuartica() {
  ...
  pasos += `<div>• Divisores de $$a = ${a}$$ ($$q$$): {${qList.join(', ')}}</div>`;
  pasos += `<div>• Raíces racionales candidatas ($$p/q$$): {${candidatos.map(v => Number(v.toFixed(2))).join(', ')}}</div>`;
